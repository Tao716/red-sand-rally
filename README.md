...
**[点击在线试玩](https://tao716.github.io/red-sand-rally/)** · 无需安装或登录，推荐电脑横屏，也支持手机触屏。

试玩为单人对 AI 比赛，不是联机房间。皮肤偏好与个人纪录保存在各自浏览器中；分享这个地址不会共享你的存档，也不需要本地电脑一直开机。

![最新版大厅：午夜电光赛车与六款皮肤选择](docs/screenshots/menu-skins.jpg)

*最新版大厅：六款「沙暴 07」涂装可直接选择，点选后实时预览，并记住你的选择。*

## 本次更新

这次更新围绕「换上喜欢的涂装出发，更清楚地掌握每一次漂移与攻防」展开：

- **[六款赛车皮肤](#赛车皮肤)**：从赤沙经典到午夜电光、极地冰川、霓虹脉冲、毒液竞速和熔岩余烬，车漆、贴花、轮毂饰色与氮气尾焰成套变化。全部直接可用，只改变外观，不提供性能加成。
- **[漂移反馈更清楚](#核心玩法)**：出弯小喷与道具强化分开显示；看见「小喷就绪」后松开漂移，便能获得释放确认。没有道具时，也能清楚看到漂移的小喷收益。
- **[驾驶与战斗反馈升级](#战斗与拾取)**：高速普通转向平滑衰减，松键和反打更快响应；持有火箭时显示锁定目标，来袭风险则有独立的方位提示与护盾使用提醒。
- **[触屏提示同步](#操作方式)**：道具按钮显示实际持有物和强化状态，操作提示跟随键盘或触屏切换；小屏横屏布局为车身预览与驾驶保留空间。

![最新版比赛画面：换装后的赛车与实时比赛HUD](docs/screenshots/race-desktop.jpg)

*比赛仍然是六车三圈的峡谷竞速：选择的皮肤会带入赛道，驾驶、道具和碰撞规则保持一致。*

以下新截图均来自当前代码在浏览器中的运行画面。漂移和战斗截图使用受控演示局面展示短暂提示，界面由真实游戏逻辑生成，不是效果图或整场比赛成绩展示。
...
当前通过 **GitHub Pages** 托管：[https://tao716.github.io/red-sand-rally/](https://tao716.github.io/red-sand-rally/)。推送到 `main` 后，[发布工作流](https://github.com/Tao716/red-sand-rally/actions/workflows/pages.yml) 会安装锁定依赖、运行单元测试、构建并发布 `dist/`；测试或构建失败时不会替换线上版本。

本地开发仍使用根路径 `/`。发布时通过 `RALLY_BASE_PATH=/red-sand-rally/` 设置 GitHub 项目站点的子路径，避免脚本、字体与图标加载失败。需要复现生产环境时：

```sh
RALLY_BASE_PATH=/red-sand-rally/ npm run build
RALLY_BASE_PATH=/red-sand-rally/ npm run preview -- --port 4173
```

预览启动后，在另一个终端运行 `node scripts/smoke-site.mjs`，可检查生产版的资源路径、皮肤保存、键盘驾驶与触屏输入。设置 `RALLY_SMOKE_URL=https://tao716.github.io/red-sand-rally/` 可对公开试玩页做同样的检查；脚本使用独立浏览器会话，不读取个人账号或日常存档。
...
![手机横屏驾驶：左右转向、刹车、漂移、氮气、道具与油门按钮](docs/screenshots/race-mobile.jpg)

*手机横屏实机画面。虚拟按键支持多指输入；持有道具时，按钮图标与强化状态同步显示。*
...
![漂移功能演示：独立的出弯小喷反馈](docs/screenshots/drift-boost.jpg)

*漂移功能演示：小喷的积累、就绪与释放不再和道具强化共用一个百分比；即使空手漂移，也能看到对应反馈。*
...
![战斗功能演示：火箭来袭方位与护盾使用提示](docs/screenshots/combat-warning.jpg)

*受控来袭演示：预警根据当前轨迹与接近速度判断风险，不保证攻击一定命中；仍需结合护盾状态、路线和障碍做出应对。*
...
| 极地冰川 | 熔岩余烬 |
| --- | --- |
| ![极地冰川：冰白车身、深蓝箭纹与蓝色轮毂饰圈](docs/screenshots/skin-glacier.jpg) | ![熔岩余烬：深红车身、暖金斜纹与金色轮毂饰圈](docs/screenshots/skin-ember.jpg) |

*同一大厅机位的局部实机截图。封面展示午夜电光，以上两款展示不同的配色与图案；六款均可在大厅随时预览。*
...
```text
.github/workflows/
  pages.yml          main 分支的测试、构建与 GitHub Pages 发布
scripts/
  capture-readme.mjs  从当前游戏生成 README 使用的七张实机截图
  smoke-site.mjs      从公开 UI 验证生产版资源、换肤与桌面/触屏驾驶
docs/screenshots/
  *.jpg              新版大厅、皮肤、驾驶、漂移、战斗与手机横屏截图
src/
...
先用 `npm run dev` 启动本地游戏，再在另一个终端运行：

```sh
node scripts/capture-readme.mjs
```

脚本使用独立的无头 Chrome 会话，将截图写入 `docs/screenshots/`，不会更改日常浏览器中的皮肤偏好和比赛纪录。大厅、起跑与手机驾驶使用正常交互；漂移和战斗演示会布置可重复的物理场景，再通过实际输入与游戏逻辑触发提示。原有 `docs/menu.jpg` 和 `docs/race.jpg` 保留作为早期截图。
